...
*/
	...
	if(empty($rows)) {
		...
		echo "<br><div class='alert alert-danger' role='alert'> <tr>";
			echo "<td colspan='4'>En éste plan, no existen laboratorios para el diagnóstico elegido.</td>";
		...
		foreach ($rows as $row) {
			if($cont2==3){ echo "<tr>"; }
			echo '<td width="2%"><input onclick="valida_lab();" type="checkbox" name="checkboxes[]" value="'.$row['idproducto'].'"></td>';				
		...
		echo "</div>";
		echo "</div>";
		echo '<br><input type="submit" id="btn_med" name="" value="Validar Laboratorios Cubiertos" class="btn btn-custom btn-celeste" disabled>';
