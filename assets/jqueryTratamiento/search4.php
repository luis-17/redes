...
*/
	...
	if (isset($_GET['dianostico_temp'])) {
		...
		$sql = "select * from producto pr inner join producto_detalle pd on pr.idproducto=pd.idproducto inner join diagnostico_producto dp on dp.idproducto=pr.idproducto inner join diagnostico d on d.iddiagnostico=dp.iddiagnostico where codigo_cie='$resultado' and pd.idplandetalle=$idplandetalle";
	...
	if(empty($rows)) {
		...
		echo "<br><div class='alert alert-danger' role='alert'> <tr>";
			echo "<td colspan='4'>En éste plan, no existen imágenes cubiertas para el diagnóstico elegido.</td>";
			
		echo "</tr></div>";
				
	}
	else {
			 echo "<form method='post' action='https://www.red-salud.com/redes/index.php/guardar_cobertura'>";
			 echo "<br><div >";
		...
		foreach ($rows as $row) {
			if($cont2==3){ echo "<tr>"; }
			echo '<td width="2%"><input onclick="valida_rayos();" type="checkbox" name="checkboxes[]" value="'.$row['idproducto'].'"></td>';				
			echo '<td width="48%">'.$row['descripcion_prod'].'</td>';
			$cont= $cont +1; 
			if($cont==2){ 
				$cont2=3; 
				$cont=0; 
		...
		echo "</div>";
		echo "</div>";
		echo '<br><input type="submit" id="btn_med" name="" value="Validar Imágenes Cubiertos" class="btn btn-custom btn-celeste" disabled>';
