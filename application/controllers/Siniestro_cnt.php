...
class Siniestro_cnt extends CI_Controller {
	...
	public function detalle_cobertura($idplandetalle,$certase_id,$idvariableplan,$e,$estado2){
		...
		$this->load->view('templates/detalle_cobertura2.php',$data);
	}
	...
	function reimprimir_pdf($idsiniestro,$idvariableplan){
		//load session library
		$this->load->library('session');
		...
		if($this->session->userdata('user')){
			...
			if(!empty($med)){
					$diagnostico = $med['dianostico_temp'];
	       		}else{
	       			$diagnostico = "";
	       		}
	        ...
	        $this->pdf = new Pdf();
			    ...
			    $this->pdf->Ln(-2);  
	          	...
	          	$this->pdf->Ln();
	          	$this->pdf->SetFont('Arial','',10); 
	          	$this->pdf->MultiCell(0,6,utf8_decode("FORMULARIO DE ORDEN DE MADICAMENTOS"),0,'R',false);
	          	$this->pdf->Ln(10);	    	
